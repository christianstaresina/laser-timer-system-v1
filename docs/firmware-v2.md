...
(which polls the return radio path, gate sensor, heartbeats, and buzzer).
...
1. Long-press RX encoder → **Stopwatch**. Sets receiver-level `timer_state` to
   ...
   flushes the RX FIFO. It does **not** fully clear an in-progress
...
| Main item | Behavior |
| --- | --- |
| `Stopwatch` | Arms receiver timing (`timer_state = ENABLED`). |
...
- **Re-pair** enters `MS_RePairing` and shows the pairing screen until the
  re-pair handler accepts a magic-matched packet, or the user short/long-presses
  to leave. See pitfalls for interaction with the top-of-loop `PollRadio()`.
...
5. **Re-pair vs top-of-loop `PollRadio()`** — `loop()` always calls `PollRadio()`
   before `menuTick()`. During `MS_RePairing`, that can drain a valid packet
   before `tickRePairing()`’s own `radio.available()` check. `PollRadio()` does
   not advance the re-pair menu screen when a menu is active. Expect flaky
   re-pair completion until radio ownership is exclusive to the re-pair path.
6. **Incomplete stopwatch arm reset** — selecting Stopwatch sets
   `timer_state = ENABLED` and clears finish/beam edge state, but does not reset
   `gate2.timer_state` / `startMillis` / related run fields. Re-arming during or
   after a partial run can surprise timing state.
